    ...
    fetch('../admin/loadRates.php')
        ...
        .then(data => {
            ...
            for (let key in data) {
                ...
                cell4.innerHTML = `<button class="button-icon" onclick="showEditModal('${key}', ${data[key]})"><i class="fa-solid fa-pen-to-square"></i></button>`; // Edit icon/button
            }
        })
...
function saveChanges() {
    ...
    if (!isNaN(newPrice) && currentEditingKey !== null) {
        ...
        .then(data => {
            if(data.success) {
                alert(data.message);
                loadTicketRates(); // Reload the table
